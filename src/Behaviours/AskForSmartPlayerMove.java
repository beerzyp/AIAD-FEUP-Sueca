...
public class AskForSmartPlayerMove extends OneShotBehaviour {
	...
	public AskForSmartPlayerMove(Jogo Sueca,String botType) {
		sueca=Sueca;
		BotType=botType;
	...
	public AskForSmartPlayerMove(Jogo suecaGame, Jogador playerToMove,Round round) {
		sueca=suecaGame;
		c1=new Carta();
		player=playerToMove;
		currRound=round;
	}
	public AskForSmartPlayerMove(Jogo suecaGame,Round round,String botType) {
		sueca=suecaGame;
		BotType=botType;
		currRound=round;
		c1=new Carta();
	...
	public void action() {
		...
		while(!validPlay) {
			...
			if(aux[0].equals("valid")){
				...
				if(this.sueca.getGameLogic().validPlay(attempt, this.sueca.getPlayer(playerToMove), currRound)) {
